...
template <typename T> class AnySpecialization : public AnyBase {
public:
...
public:
  ...
  template <typename T> const T *TryCast() const { return ptr_->TryCast<T>(); }

  template <typename T> const T &Cast() const { return ptr_->Cast<T>(); }
