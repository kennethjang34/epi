...
using HasSerializationTraitSpecialization = std::integral_constant<
    bool, !std::is_base_of<NoSpecializationTag, SerializationTrait<T>>::value>;

/**
 * void specialization.
 * It is used when tested function has void return type.
 */
template <> struct SerializationTrait<void, void> {
 ...
 */
template <> struct SerializationTrait<float, void> {
  ...
  static std::vector<int> GetMetrics(const serialization_type &x) {
    return {static_cast<int>(std::min<serialization_type>(
        std::abs(x), (const float)std::numeric_limits<int>::max()))};
  }
 ...
 */
template <> struct SerializationTrait<double, void> {
 ...
 */
template <> struct SerializationTrait<bool, void> {
 ...
 */
template <> struct SerializationTrait<std::string, void> {
 ...
 */
template <typename Inner> struct SerializationTrait<std::vector<Inner>, void> {
...
struct SerializationTrait<std::tuple<TupleTypes...>, void> {
  ...
  template <size_t I> using ith_item_t = std::tuple_element_t<I, tuple_type>;

  template <size_t I> using ith_item_trait = SerializationTrait<ith_item_t<I>>;
...
private:
  ...
  GetMetricNamesImpl(const std::string &arg_name,
  ...
  template <size_t... I>
  static serialization_type
  JsonArrayToTuple(const json &json_arr, std::index_sequence<I...> /*unused*/) {
 ...
 */
template <bool HasParent> struct BinaryTreeSerializationTraitHelper {
  template <typename Node, typename Parent>
  static void InitParent(const Node &n, const Parent &p) {}
};

template <> struct BinaryTreeSerializationTraitHelper<true> {
  template <typename Node, typename Parent>
...
struct BinaryTreeSerializationTrait {
  ...
  static const char *Name() {
    static std::string s = FmtStr("binary_tree({})", inner_type_trait::Name());
    return s.c_str();
  }
  ...
  static serialization_type Parse(const json &json_object) {
    ...
    for (const auto &x : json_object) {
      nodes.emplace_back(x.is_null() ? nullptr
                                     : new node_t{inner_type_trait::Parse(x)});
    }
...
struct UserSerTrait : SerializationTrait<std::tuple<Members...>> {
  ...
  template <size_t... I>
  static serialization_type
  FromTupleDefaultImpl(const std::tuple<Members...> &values,
